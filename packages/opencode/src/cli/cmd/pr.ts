...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run arcanea-code",
  builder: (yargs) =>
  ...
  async handler(args) {
    await Instance.provide({
      ...
      async fn() {
          ...
          if (prInfoText.trim()) {
            ...
            if (prInfo && prInfo.body) {
              ...
              if (sessionMatch) {
                ...
                UI.println(`Found arcanea-code session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["arcanea-code", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting arcanea-code...")
        UI.println()

        const opencodeArgs = sessionId ? ["-s", sessionId] : []
        const opencodeProcess = Process.spawn(["arcanea-code", ...opencodeArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
